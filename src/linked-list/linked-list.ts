import { doubleNode, simpleNode, weigthNode } from './node';
...
export class WeightedLinkedList<T> {
 public head: weigthNode<T> | null; 
  public tail: weigthNode<T> | null;
  public length: number;
  ...
  public append(value: T, weight: number): void {
    const newNode = new weigthNode(value, weight);
  ...
  public prepend(value: T, weight: number): void {
    const newNode = new weigthNode(value, weight);
  ...
  public find(value: T): weigthNode<T> | null {
    if (this.head === null) {
      return null;
    }
    return this.findNode(this.head, value);
  }
  private findNode(node: weigthNode<T>, value: T): weigthNode<T> | null {
    if (node.value === value) {
      return node;
    }
    if (node.next === null) {
  ...
  private removeNode(node: weigthNode<T>, value: T): boolean {
    if (node.next === null) {
      return false;
    }
    if (node.next.value === value) {
  ...
  public show(): void {
    console.log(`Weighted Linked List: ${this.length} nodes`);
    if (this.head === null) {
      console.log('Empty list');
      return;
    }
  ...
  private showNode(node: weigthNode<T>): void {
    process.stdout.write(`${node.value}(${node.weight}) -> `);
    if (node.next === null) {
      process.stdout.write('null');
      return;
    }
